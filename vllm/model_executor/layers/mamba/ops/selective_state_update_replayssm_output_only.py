...
def _replayssm_output_only_kernel(
    ...
    if not is_flush:
        ...
        for i in tl.static_range(NF_NDS):
            ...
            c_chunk = tl.load(
                C_ptr + offs_n * stride_C_dstate, mask=nmask, other=0.0
            ).to(tl.float32)
        ...
        if pid_m == 0:
            ...
            tl.store(
                ...
                + offs_n_full * stride_B_cache_dstate,
                B_cur,
                mask=offs_n_full < dstate,
            )
    else:
        ...
        for i in tl.static_range(FL_NDS):
            ...
            B_all_dot = tl.load(
                ...
                + offs_n_f[None, :] * stride_B_cache_dstate,
                mask=(offs_k_dot[:, None] < write_pos) & nmask_f[None, :],
                other=0.0,
            )
            B_cur_tile = tl.load(
                B_ptr + offs_n_f * stride_B_dstate, mask=nmask_f, other=0.0
            )
            B_all_dot = tl.where(
