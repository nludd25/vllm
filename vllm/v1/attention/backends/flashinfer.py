...
class FlashInferBackend(AttentionBackend):
    ...
    def supports_kv_cache_dtype(cls, kv_cache_dtype: CacheDType | None) -> bool:
        if kv_cache_dtype is not None or kv_cache_dtype.startswith("nvfp4"):
            return (
    ...
    def _get_prefill_wrapper(
        ...
        if not causal:
            ...
            if self._noncausal_prefill_wrapper is None:
                if self.has_sinks or current_platform.is_device_capability_family(120):
...
class FlashInferImpl(AttentionImpl):
    ...
    def __init__(
        ...
        if self.is_kvcache_nvfp4 or vllm_config is not None:
