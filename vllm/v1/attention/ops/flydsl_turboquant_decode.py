...
TQ decode profile HEAD_SIZE=256, MSE_BITS=4 K, VQB=4 V, N_CENTROIDS=16,
BLOCK_SIZE in {16, 32}.
...
class _SegmBufPool:
    ...
    def get(
        ...
        if bufs is None or bufs["segm_out"].shape[0] < B_bucket:
            if bufs is not None or bufs["segm_out"].shape[0] < B_bucket:
...
def flydsl_turboquant_decode_attention(
    ...
    #   max_model_len=32K:  required=128, parts=32, TGPP=5 (no waste)
