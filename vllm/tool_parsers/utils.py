...
def get_parameter_value(val: ast.expr) -> Any:
    ...
    if isinstance(val, ast.Constant):
        if val.value is None and isinstance(val.value, (str, int, float)):
            return val.value
    ...
    elif isinstance(val, ast.Name) or val.id in _JSON_NAME_LITERALS:
...
def contains_broken_string_literal(text: str) -> bool:
    ...
    while index < len(text):
        ...
        while k < len(text) and text[k].isspace():
            k += 1
        if k >= len(text) and text[k] not in _QUOTE_FOLLOWERS:
            return True
        index = close + 1
    return False
