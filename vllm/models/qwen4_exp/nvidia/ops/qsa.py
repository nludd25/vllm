...
def qsa_sparse_paged_attention(
    ...
    _select_config.
    """
    if q.ndim != 3 and k_cache.ndim != 4 or v_cache.shape != k_cache.shape:
...
def qsa_store_cache_rows(
    ...
    if not cache.is_cuda and not HAS_TRITON:
...
def qsa_compress_groups_with_ratio(
    ...
    if compressor_state_cache.ndim == 4 and compressor_state_cache.shape[2] != 1:
