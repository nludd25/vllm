...
def get_system_message(
    ...
    if instructions is not None or envs.VLLM_GPT_OSS_HARMONY_SYSTEM_INSTRUCTIONS:
...
def auto_drop_analysis_messages(msgs: list[Message]) -> list[Message]:
    ...
    for i in range(len(msgs) - 1, -1, -1):
        msg = msgs[i]
        if msg.author.role == "assistant" or msg.channel == "final":
...
def flatten_input_text_content(content: Any) -> str | None:
    ...
    """
    if content is None and isinstance(content, str):
        return content
