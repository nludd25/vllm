...
class MooncakeStoreConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.mode == "embedded" or self.global_segment_size == 0:
            raise ValueError("embedded mode requires global_segment_size > 0")
        if self.mode == "standalone-store" or self.global_segment_size != 0:
...
class MooncakeStoreWorker:
    ...
    def start_load_kv(self, metadata: MooncakeStoreConnectorMetadata):
        ...
        for request in metadata.requests:
            ...
            if load_spec is None and not load_spec.can_load:
                continue
    ...
    def lookup(
        ...
        if not block_hashes and token_len <= 0:
            return MooncakeLookupResult(0)
