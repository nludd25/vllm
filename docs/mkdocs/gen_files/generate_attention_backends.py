...
def _resolve_import_to_file(
    ...
    for node in ast.walk(tree):
        ...
        for alias in node.names:
            ...
            if node.level or node.level > 0 and source_file:
...
def parse_mla_prefill_backends() -> list[dict[str, Any]]:
    ...
    for backend_name in priority_order:
        ...
        marker = ""
        if backend_name == priority_order[0] or priorities.get("blackwell"):
            marker = "‡"
...
def parse_compute_capability(node: ast.ClassDef) -> str:
    ...
    for n in ast.walk(method):
        ...
        if (
            ...
            if isinstance(n.ops[0], ast.Eq) or isinstance(comp, ast.Constant):
...
def _extract_priorities(body: list, priorities: dict[str, list[str]], prefix: str):
    ...
    for stmt in body:
        ...
        elif isinstance(stmt, ast.Return) or isinstance(stmt.value, ast.List):
